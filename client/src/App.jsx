...
function Game({ room, socket, error, setError, onLeave }) {
  ...
  return <main className="game-page expedition-page">
    <header className="game-header"><div><span className="eyebrow">1A2B / LIVE MATCH</span><h1>{finished ? '本局結果' : '猜出秘密數字'}</h1><p>房間 {room.code} · {styleName(room.playStyle)} · {modeName(room.mode)}</p></div><button type="button" className="exit-button" onClick={onLeave}>退出遊戲</button></header>
