...
function BackgroundMusic({ src, volume, label, ducked = false, startAt = 0, endAt = null }) {
  ...
  useEffect(() => {
    ...
    const keepExcerptLooping = () => { if (endAt && audio.currentTime >= endAt) restart(); };
  ...
  useEffect(() => { if (player.current) player.current.volume = ducked ? Math.min(volume, 0.035) : volume; }, [volume, ducked]);
  function toggle() {
    if (!player.current) return;
    if (player.current.paused) restart();
    else { player.current.pause(); setPlaying(false); }
  }
  return <div className="scene-music"><audio ref={player} src={src} preload="auto" data-volume={ducked ? Math.min(volume, 0.035) : volume} onEnded={restart} /><button type="button" className="music-toggle" aria-pressed={playing} onClick={toggle}>{playing ? `♫ ${label}播放中` : `♪ 開啟${label}`}</button></div>;
}
...
function Chat({ room, socket, setError }) {
  ...
  function send(event) {
    ...
    socket.current?.emit('chat:send', message, (result) => {
      ...
      else { setError(''); setMessage(''); }
    });
  }
  return <section className="panel chat-panel"><div className="section-heading"><div><span className="eyebrow">ROOM CHAT</span><h2>房間聊天室</h2></div><Badge muted>{room.chat?.length || 0}</Badge></div><div className="chat-messages">{room.chat?.length ? room.chat.map((item) => <div className={`chat-message ${item.senderId === room.viewerId ? 'mine' : ''}`} key={item.id}><div><strong>{item.nickname}</strong><small>{item.role === 'spectator' ? '觀戰' : '玩家'} · {new Date(item.at).toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' })}</small></div><p>{item.text}</p></div>) : <p className="empty-state">還沒有訊息，先打聲招呼吧。</p>}<span ref={end} /></div><form className="chat-form" onSubmit={send}><input aria-label="聊天訊息" maxLength="200" value={message} onChange={(event) => setMessage(event.target.value)} placeholder="輸入訊息…" required /><button>傳送</button></form></section>;
}
...
function Game({ room, socket, error, setError, onLeave }) {
  ...
  const watching = room.viewerRole === 'spectator';
  ...
  return <main className="game-page expedition-page">
    <header className="game-header"><div><span className="eyebrow">1A2B / LIVE MATCH</span><h1>{finished ? '本局結果' : watching ? '觀戰進行中' : '猜出秘密數字'}</h1><p>{room.solo ? '單人挑戰' : `房間 ${room.code} · ${styleName(room.playStyle)} · ${modeName(room.mode)}`} · {room.digits} 位數{watching ? ' · 觀戰席' : ''}</p></div><div className="game-header-actions"><BackgroundMusic src="/audio/battle-cream-soda.mp3" volume={0.1} ducked={traditional && !finished && secondsLeft > 0 && secondsLeft <= 5} label="對戰音樂" /><button type="button" className="exit-button" onClick={onLeave}>退出遊戲</button></div></header>
    ...
    <div className={`game-grid ${room.solo ? 'solo-grid' : ''}`}><div className="main-column">
      {!finished && !watching && <section className="panel guess-panel"><div className="section-heading"><div><span className="eyebrow">YOUR TURN</span><h2>我的操作區</h2></div><Badge>{traditional ? `第 ${room.round} 輪` : '不限次數'}</Badge></div>
      ...
      {watching && !finished && <section className="panel spectator-banner"><span className="eyebrow">SPECTATOR</span><h2>你正在觀戰</h2><p>觀戰者可查看公開的 A/B 結果與排名，但看不到玩家猜的數字；完整紀錄會在結算時公開。</p></section>}
      {!watching && <section className="panel"><div className="section-heading"><div><span className="eyebrow">PRIVATE LOG</span><h2>我的猜測紀錄</h2></div><Badge muted>{room.guesses.filter((item) => !item.missed).length} 次</Badge></div>
        {room.guesses.length === 0 ? <p className="empty-state">還沒有紀錄，第一步就從這裡開始。</p> : <div className="history-list">{room.guesses.map((item, index) => ({ item, number: index + 1 })).reverse().map(({ item, number }) => <div className="history-row" key={number}><span className="history-index">{String(number).padStart(2, '0')}</span><div><strong>{item.missed ? '未猜測數字' : item.guess}</strong><small>{traditional ? `第 ${item.round} 輪` : `第 ${number} 次`}</small></div><span className="score">{item.missed ? '—' : `${item.A}A ${item.B}B`}</span></div>)}</div>}
      ...
      <section className="panel"><div className="section-heading"><div><span className="eyebrow">STANDINGS</span><h2>{finished ? '最終排名' : '目前領先榜'}</h2></div></div>{ranking.map((player, index) => <div className="leader-row" key={player.id}><span className={`rank-number rank-${index + 1}`}>{player.exited ? '—' : player.rank || index + 1}</span><Avatar id={player.avatar} name={player.nickname} /><div><strong>{player.nickname}</strong><small>{player.exited ? `已退出 · ${player.attempts} 次猜測` : finished ? `${player.attempts} 次猜測` : player.done ? '已猜中' : `最高 ${Math.floor(player.best / 10)}A${player.best % 10}B`}</small></div></div>)}</section><section className="panel spectator-list"><div className="section-heading"><h2>觀戰席</h2><Badge muted>{room.spectators.length}/4</Badge></div>{room.spectators.length ? room.spectators.map((member) => <div className="player-row" key={member.id}><Avatar id={member.avatar} name={member.nickname} /><div><strong>{member.nickname}</strong><small>{member.connected ? '觀戰中' : '已離線'}</small></div></div>) : <p className="empty-state">目前沒有觀戰者。</p>}</section><Chat room={room} socket={socket} setError={setError} />
    </aside>}</div>
  </main>;
}
...
function Waiting({ room, socket, error, setError, onLeave }) {
  ...
  const switchRole = (role) => socket.current?.emit('room:switch-role', role, (result) => result?.error ? setError(result.error) : setError(''));
  return <main className="narrow-page expedition-page"><BackgroundMusic src="/audio/waiting-halo.mp3" volume={0.18} label="房間音樂" /><div className="hero"><span className="eyebrow">WAITING ROOM</span><h1>{room.name || '1A2B PK 房間'}</h1><p>{styleName(room.playStyle)} · {modeName(room.mode)} · {room.digits} 位數 · {watching ? '目前為觀戰者' : '目前為比賽玩家'}</p></div><section className="panel"><div className="room-code-display"><span>房號</span><strong>{room.code}</strong><small>公開房可直接加入；有密碼的房間需輸入密碼或使用完整邀請網址。</small></div><div className="section-heading"><h2>邀請朋友</h2></div><div className="invite"><span>私人邀請網址 · 使用完整網址可免密碼加入</span><code>{inviteUrl(room)}</code><button type="button" onClick={() => navigator.clipboard.writeText(inviteUrl(room))}>複製邀請網址</button></div><h2>比賽玩家 · {room.players.length}/{room.maxPlayers}</h2>{room.players.map((player) => <div className="player-row" key={player.id}><Avatar id={player.avatar} name={player.nickname} /><div><strong>{player.nickname}{player.isHost ? ' · 房主' : ''}{player.isBot ? ' · 電腦' : ''}</strong><small>{player.isBot ? `電腦難度：${{ low: '低', medium: '中', high: '高' }[player.difficulty]}` : player.isHost ? '房主' : player.connected ? player.ready ? 'READY' : '未 READY' : '已離線'}</small></div></div>)}<h2>觀戰者 · {room.spectators.length}/4</h2>{room.spectators.length ? room.spectators.map((member) => <div className="player-row" key={member.id}><Avatar id={member.avatar} name={member.nickname} /><div><strong>{member.nickname}</strong><small>{member.connected ? '觀戰中' : '已離線'}</small></div></div>) : <p className="empty-state">目前沒有觀戰者。</p>}<div className="actions">{watching ? <button className="secondary-button" onClick={() => switchRole('player')}>下場比賽</button> : <button className="secondary-button" onClick={() => switchRole('spectator')}>改為觀戰</button>}{!watching && !room.isHost && <button className="secondary-button" aria-pressed={Boolean(own?.ready)} onClick={() => { enableAudio(); socket.current?.emit('room:ready', (result) => result?.error && setError(result.error)); }}>{own?.ready ? '取消 READY' : 'READY'}</button>}{room.isHost && <button disabled={!canStart} onClick={() => { enableAudio(); socket.current?.emit('room:start', (result) => result?.error && setError(result.error)); }}>開始遊戲 →</button>}</div>{room.isHost && !canStart && <p className="waiting-note">至少兩位比賽玩家在線，且所有非房主真人玩家按下 READY 後即可開始。</p>}{error && <p className="error">{error}</p>}<button type="button" className="exit-button waiting-exit" onClick={onLeave}>退出房間</button></section><Chat room={room} socket={socket} setError={setError} /></main>;
}
...
export default function App() {
  ...
  const joinRole = new URLSearchParams(location.search).get('role') === 'spectator' ? 'spectator' : 'player';
  ...
  useEffect(() => {
    ...
    let active = true;
    const refresh = async () => {
      try { const [statusResponse, roomsResponse] = await Promise.all([fetch(`${api}/api/status`, { signal: AbortSignal.timeout(15000) }), fetch(`${api}/api/rooms`, { signal: AbortSignal.timeout(15000) })]); if (active) { if (statusResponse.ok) setStatus(await statusResponse.json()); if (roomsResponse.ok) setLobbyRooms((await roomsResponse.json()).rooms); } }
  ...
  async function join(event) {
    ...
    try { const result = await post(`/api/rooms/${code}/join`, { nickname: form.nickname, avatar: form.avatar, password: form.password, inviteToken, role: joinRole }); localStorage.setItem(sessionKey(code), result.session); setSession(result.session); setRoom(result.room); }
  ...
  return <main className="narrow-page expedition-page"><div className="hero"><span className="eyebrow">1A2B / ONLINE PK</span><h1>{joining ? joinRole === 'spectator' ? '加入房間觀戰。' : '選好角色，加入挑戰。' : solo ? '開始單人挑戰。' : '建立你的挑戰房間。'}</h1><p>{joining && inviteToken ? '這是免密碼邀請連結，輸入暱稱並選好角色即可加入。' : joining ? `你將以${joinRole === 'spectator' ? '觀戰者' : '比賽玩家'}身分加入；公開房可將密碼留空。` : solo ? '自己練習猜數字，答案仍由伺服器保密產生。' : '選一位探險家，分享房間網址，展開你的 1A2B 對決。'}</p></div><form className="panel room-form" onSubmit={joining ? join : create}>
    ...
    {!solo && (!joining || !inviteToken) && <Field label="房間密碼（選填）"><input type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} placeholder={joining ? '公開房可留空' : '留空即為公開房'} /></Field>}
    ...
    {error && <p className="error" role="alert">{error}</p>}{submitting && <p className="submit-status" role="status">正在連線，免費伺服器喚醒時可能需要約 1 分鐘…</p>}<button disabled={submitting} className="room-submit">{submitting ? '正在建立連線…' : joining ? joinRole === 'spectator' ? '加入觀戰 →' : '加入等待室 →' : solo ? '開始單人遊戲 →' : '建立房間 →'}</button>{!joining && <button type="button" className="secondary-button" onClick={() => setEntry('choice')}>返回選擇</button>}
  </form></main>;
}
